...
import { upsertEntry, deleteEntry, JOURNAL_KEY, type JournalEntry } from "@/lib/journal";
import AttachComponent, { type Attachment, type HouseData } from "@/components/journal/AttachComponent";
import { type ScoredTransit } from "@/lib/transitFilter";
import { type ChartConfiguration } from "@/lib/configurations";
...
/**
 * Reconstruct an Attachment from context_data stored in localStorage.
 * JSON.stringify converts Date objects to ISO strings; this re-hydrates them.
 */
function reconstructAttachment(
  ...
  if ("transit" in contextData) {
    ...
    const raw = contextData.transit as any;
    return {
      ...
      data: {
        ...
        transit: {
          ...raw.transit,
          peakDate: new Date(raw.transit.peakDate as string),
        },
      } as ScoredTransit,
    };
  }
  ...
  if ("house" in contextData) {
    return { type: "house" as const, data: contextData.house as HouseData };
  }

  if ("configuration" in contextData) {
    return {
      type: "configuration" as const,
      data: contextData.configuration as ChartConfiguration,
    };
  }
...
export default function JournalEntryPage() {
  ...
  const [isEditMode, setIsEditMode]     = useState(false);   // view mode on load
  const [showBurnModal, setShowBurnModal] = useState(false);
  const [attachment, setAttachment]     = useState<Attachment | null>(null);

  // Stable entry ID — mirrors URL param, set once on mount
  ...
  // ─── Mount: load entry from localStorage ────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const att = reconstructAttachment(found.context_data);
    if (att) {
      attachmentRef.current = att;
      setAttachment(att);
    }
    savedEntryRef.current = found;
    setSavedEntry(found);
  // id is the URL param for this page and never changes after mount;
  // router from useRouter() is stable across renders.
